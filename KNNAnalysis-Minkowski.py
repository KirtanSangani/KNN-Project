...
knn_cv = KNeighborsClassifier(n_neighbors = 15, metric = "minkowski")
scaler = StandardScaler()
scaler.fit(X)
X = scaler.transform(X)
...
for k in k_range:
    knn_cv = KNeighborsClassifier(n_neighbors=k, metric = "minkowski")
    cv_scores = cross_val_score(knn_cv,X,np.ravel(y,order='C'),cv=10)
    print(k)
    print(cv_scores)
    print(np.mean(cv_scores))
...
for i in range(1,15):
    knn = KNeighborsClassifier(i, metric = "minkowski")
